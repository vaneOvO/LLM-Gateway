...
async function loadConfig(env) {
  try {
    ...
    return { endpoints: [], fallbackModels: [] };
  }
}
...
export async function onRequestPut({ request, env }) {
  ...
  await env.CONFIG_KV.put("config", JSON.stringify({ endpoints: clean, fallbackModels }));
  return j({ ok: true, endpoints: clean, fallbackModels });
}
