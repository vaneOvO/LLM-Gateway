...
async function handle({ request, env }) {
  ...
  // 同步完模型清单后：如果开了 autoSelfTest，用网关自身的 /v1 测活（享受路由 + 故障转移 + 加权随机，
  // 和前端「魔法全库测活」机制完全一致）；失败自动重试一次，避免临时 503/429 误判。
  ...
  if (ss.autoSelfTest) {
    ...
    const gatewayKey = env.PROXY_API_KEY || "";
    ...
    async function probeOne(model) {
      ...
      try {
        let r;
        if (gatewayKey) {
          // 走自身 /v1 —— 网关会按延迟加权选一家、失败自动切换，跟前端全库测活一致
    ...
    async function tWorker() {
      while (tQueue.length > 0) {
        ...
        await acquire();
        let res = await probeOne(m);
        if (!res) { await acquire(); res = await probeOne(m); } // 重试一次（与前端一致）
    ...
    for (let k = 0; k < Math.min(testConc, list.length); k++) tws.push(tWorker());
    await Promise.all(tws);

    alive.sort((a, b) => a.lat - b.lat);
    fallbackModels = alive.map((x) => x.model);
